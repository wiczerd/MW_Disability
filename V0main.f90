...
!cleans everything except z_jt_panel,z_jt_macroint
	...
	subroutine alloc_shocks(shk)
		...
		allocate(shk%drawi_ititer(Nsim,2000), stat=shk%alloced)
		allocate(shk%drawt_ititer(Nsim,2000), stat=shk%alloced)
...
!								&  (status_it(drawi_ititer(i,ii+1),drawt_ititer(i,ii+1)) .gt. 0) .and. (status_it(drawi_ititer(i,ii+1),drawt_ititer(i,ii+1)) .le. 3) ) then
								...
								if(ii>=Ncol) then
									nomatch = nomatch+1
									print *, "missed at d_hr: ", d_hr, "t= ", it, "al_int ", al_int_it(i,it), "del_i_int ",del_i_int(i), " j_i= ", j_hr
								endif
								exit
							endif
						enddo
...
program V0main
	...
	! NLopt stuff
		...
		real(dp) :: lb(3),ub(3),parvec(3), ervec(3), erval, param0(3)=1.,err0(3)=1.,lb_1(1),ub_1(1),parvec_1(1),ervec_1(1)
		integer  :: nodei,ierr
...
!	occ_dat = .false.
	...
	c1 = size(X1,dim=2)
	c2 = size(X1,dim=1)
	call dgemm('T','N',c1,c1,c2,t1,X1,c2,X2,c2,t2,X3,c1)
	print *, X3
...
!	set mean wage:
 ...
 ! 	 	write(fcallog, "(G20.12)", advance='no')  xizcoef
	...
	if( (run_cal .eqv. .true.) .and. (dbg_skip .eqv. .false.) ) then
		...
		call cal_mlsl( erval, parvec, 3, lb, ub, shk)
